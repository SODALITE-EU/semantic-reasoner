...
//	String ws = "http://";
	...
	public IRI start() throws MappingException, ValidationException  {
		...
		for (Resource _template : aadmModel.filter(null, RDF.TYPE, factory.createIRI(KB.EXCHANGE + "Template"))
			...
			LOG.info("namespaceOfType=" + this.namespacesOfType + "that are inherited from templateType=" + templateType);
		...
		if (inputs.size() > 0) {
			...
			for (Resource _input : inputs) {
				...
				if (!_inputName.isPresent())
					...
					LOG.info(String.format("Input name: %s", inputName));
				}
...
//			builder.add(propertyClassifierKB, factory.createIRI("dul:hasParameter"), root);
			...
			if (triggerName != null && (triggerName.equals("node"))) {
				...
				if (kbNode == null) {
					if (templateNames.contains(n.getResource()))
						...
						LOG.info(currentTemplate + ": Cannot find template: " + value.getLabel() + " for trigger " + triggerName);
					}
				}
				if(kbNode != null)
				...
				if (req_cap != null) {
					...
					LOG.warning(currentTemplate + ": Cannot find : " + value.getLabel() + " for trigger " + triggerName);
				}		
			} else {
				Object i = null;
		...
		for (Literal l:listValues) {
			...
			if (kbNode == null) {
				if (templateNames.contains(n.getResource()))
					...
					LOG.warning(currentTemplate + ": Cannot find Node: " + l.getLabel());
				}
			}
			if(kbNode != null)
	...
	private IRI getKBTemplate(NamedResource n) {
		LOG.info("getKBTemplate label = " + n.getResource() + ", namespace = " + n.getNamespace());
